...
func TestGitHubProjectContract(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/repos/acme/api":
			_, _ = io.WriteString(w, `{"full_name":"acme/api","ssh_url":"git@github.com:acme/api.git","html_url":"https://github.com/acme/api","default_branch":"trunk"}`)
	...
	if err != nil || !project.Exists || project.SSHURL == "" || project.WebURL == "" || project.DefaultBranch != "trunk" {
...
func TestGitLabProjectContract(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/projects/acme/group/api":
			_, _ = io.WriteString(w, `{"path_with_namespace":"acme/group/api","ssh_url_to_repo":"git@gitlab.com:acme/group/api.git","web_url":"https://gitlab.com/acme/group/api","default_branch":"trunk"}`)
		...
		case r.Method == http.MethodPost && r.URL.Path == "/projects":
			...
			_, _ = io.WriteString(w, `{"path_with_namespace":"acme/group/new-api","ssh_url_to_repo":"git@gitlab.com:acme/group/new-api.git","web_url":"https://gitlab.com/acme/group/new-api","default_branch":"main"}`)
		default:
			http.NotFound(w, r)
		}
	}))
	...
	if err != nil || project.Project != "acme/group/new-api" || project.DefaultBranch != "main" {
		t.Fatalf("project=%+v err=%v", project, err)
	}
}
