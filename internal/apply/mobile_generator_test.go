...
func TestMobileApplyPreservesFlutterCLIOutput(t *testing.T) {
	...
	wantFiles := map[string]string{
		...
		"pubspec.lock":          "lock generated by Flutter pub get\n",
...
if [ "${2:-}" = "pnpm" ] && [ "${3:-}" = "install" ]; then
  printf 'lock generated by pnpm install\n' > "$PWD/pnpm-lock.yaml"
  mkdir -p "$PWD/node_modules"
  printf 'installed by pnpm\n' > "$PWD/node_modules/.smt-installed"
  exit 0
fi
if [ "${2:-}" = "go" ] && [ "${3:-}" = "mod" ] && [ "${4:-}" = "download" ]; then
  printf 'setup=go mod download cwd=%s\n' "$PWD" >> "$log"
  exit 0
fi
if [ "${2:-}" = "flutter" ] && [ "${3:-}" = "pub" ] && [ "${4:-}" = "get" ]; then
  printf 'lock generated by Flutter pub get\n' > "$PWD/pubspec.lock"
  mkdir -p "$PWD/.dart_tool"
  printf 'resolved by Flutter pub get\n' > "$PWD/.dart_tool/package_config.json"
  exit 0
fi
destination=""
if [ "${2:-}" = "npx" ]; then
...
func readFakeASDFInvocation(t *testing.T, logPath string) (string, []string, string) {
	...
	for _, line := range strings.Split(strings.TrimSpace(string(contents)), "\n") {
		...
		case strings.HasPrefix(line, "cwd="):
			flush()
			current.cwd = strings.TrimPrefix(line, "cwd=")
		case strings.HasPrefix(line, "tool-versions="):
			current.versions = strings.TrimPrefix(line, "tool-versions=")
	...
	for _, call := range calls {
		...
		if strings.Contains(args, "exec\x00flutter\x00") && strings.Contains(args, "\x00create\x00") {
			return call.cwd, call.args, call.versions
		}
	}
	if len(calls) == 0 {
		return "", nil, ""
	}
	return calls[0].cwd, calls[0].args, calls[0].versions
}
