...
func TestWebApplyPreservesCLIOutputMergesIgnoreAndBootstrapsDependencies(t *testing.T) {
	logPath := installFakeNextASDF(t, false)
	parent := t.TempDir()
	destination := filepath.Join(parent, "workspace")
	raw := blueprintBytes()
	...
	if got := strings.Count(string(log), "cwd="); got != 2 {
		t.Fatalf("Apply invoked asdf %d times, want initializer plus dependency setup: %s", got, log)
	}
...
func TestNonWebApplyDoesNotInvokeNextInitializerOrEmitWebOutput(t *testing.T) {
	...
	if strings.Contains(string(contents), "arg=npx\n") {
		t.Fatalf("non-Web Apply invoked the Next.js initializer: %s", contents)
	}
...
if [ "${1:-}" = "exec" ] && [ "${2:-}" = "pnpm" ] && [ "${3:-}" = "install" ]; then
