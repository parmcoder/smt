...
const rootReadmeBase = "# Platform workspace\n\nStart with [the documentation workspace](docs/README.md). Agents also read `AGENTS.md`.\n\nBeads configuration is tracked with the workspace; its embedded Dolt database and local runtime files stay on this machine and are ignored by Git.\n\nApply bootstraps dependencies for every selected project component before publication. E2E package dependencies, browsers, devices, Podman images, and services remain explicit runtime setup.\n\n## Security verification\n\nRun `task security` as an explicit operator lane after installing OSV-Scanner 2.4.0 and Gitleaks 8.30.1. The generated lanes check selected lockfiles, runtime image and Compose hardening, and redacted secret findings. They are intentionally separate from `task verify:fast` and `task verify`; Apply does not install tools, fetch vulnerability databases, scan dependencies, or execute containers.\n\nThis v0.1.0 contract does not generate SBOMs, signatures, attestations, remote CI configuration, or E2E package dependencies. Those remain separate future or operator-owned lanes.\n"
...
func rootReadme(ociSelected bool, identitySelected ...bool) string {
	...
	readme := rootReadmeBase + "\n## Local Compose\n\nThe root Compose workflow uses the operator-managed `.env` explicitly. Apply\ndoes not generate `.env`; initialize it from the examples and set every required\ncredential before starting services. When Web is selected, its project\ndependencies and lockfile are already staged by Apply; after setting `.env` the\nbuild can run immediately:\n\n~~~sh\ncp .env.example .env\n# set DATABASE_PASSWORD and any selected identity credentials, then run:\ntask compose:config\ntask compose:build\ntask compose:up\n~~~\n\nUse `task compose:down` to stop and remove the Compose application. The root\n`.env` remains ignored and local-only.\n"
