...
func TestPlanSelectsChangedChildrenBeforeRootAndRequiresAssignedReferences(t *testing.T) {
	...
	plan, err := Plan(context.Background(), plannerConfig(root), plannerManifest(root), "feature", root, runner, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlanRejectsChangedChildWithoutRootGitlink(t *testing.T) {
	...
	_, err := Plan(context.Background(), plannerConfig(root), plannerManifest(root), "feature", root, runner, nil)
...
func TestPlanRejectsWrongAssignedReferenceAndPathMismatch(t *testing.T) {
	...
	if _, err := Plan(context.Background(), plannerConfig(root), manifest, "feature", root, runner, nil); err == nil || !strings.Contains(err.Error(), "path does not match") {
