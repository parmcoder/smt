...
Apply by `asdf exec flutter pub get` after the CLI's `--no-pub` step. For
...
workflow. After the root `.tool-versions` pins `nodejs 24.18.0` and `pnpm
11.24.0`, Web
...
Apply merges the CLI `.gitignore`, then runs `asdf exec pnpm install` in the
staged Web child after SMT files are written. If pnpm blocks dependency build
scripts, Apply runs `asdf exec pnpm approve-builds --all` and retries. A failed initializer or dependency
setup leaves the
...
E2E package dependencies and browsers remain explicit runtime setup.
...
reports unavailable Node, pnpm, browser, or platform lanes explicitly. Apply
creates the Web lockfile and dependencies; the later `.3.2.2/.3` work owns
quality, browser, and runtime verification.
